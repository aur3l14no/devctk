...
There is no daemon. The systemd unit runs `devctk apply --yes --autostart-only` at login/boot. Autostart mode only starts containers marked `systemd = true` and **never destroys anything** — destructive changes are reserved for an explicit `devctk apply`.
...
```sh
podman exec -it myproj-agent bash -lc "cd /home/y/Projects/myproj && claude --resume"
```
